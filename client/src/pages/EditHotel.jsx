...
export default function EditHotel() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(true);
      const res = await fetch(`/api/hotel/updateHotel/${currentUserData._id}`, // ruta za update i prosledjujemo id korisnika za proveru da li on ima pravo da menja taj profil
      {  
        method: 'POST',
        headers: {
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      <h1 className='text-3xl text-white font-semibold text-center my-7'>Edit Hotel</h1>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 text-white'>
        <input onChange={(e) => setFile(e.target.files[0])} type='file' ref={fileRef} hidden/>
        <img onClick={() => fileRef.current.click()} className='rounded-full h-24 w-24 object-cover cursor-pointer self-center mt-2' src={formData.logo || currentUserData.logo} alt="profile"/>
        ...
        <input type='text' id='hotelName' defaultValue={currentUserData.hotelName} onChange={handleChange} className='border p-3 rounded-lg '/>
